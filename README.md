...
- **关卡模式（2000 关）**：前 50 关手工设计、渐进式引导每个机关登场；51 关起由种子化程序生成（确定性——同一关卡永远相同），每 20 关一个星系章节（每星系专属星云配色），每关 1–3 星评价，星座星图选关（含开局预览弹窗）。
- **无尽模式**：默认棋盘变少自动补行；开启「天花板下压 ×1.5」等变异修饰符（第五色/短瞄准线/窄场地）换取分数倍率；不时有彗星横穿，击中奖励彩虹/炸弹弹药。
- **每日挑战**：按日期种子生成的每日专属关卡 + 连签日历。
- **手感**：瞄准引导线随进度递减（20 关内全长，200 关后收敛到定值），长按精准瞄准 +50% 长度并进入慢动作；点「下一个泡泡」或「⇄ 交换」按钮换弹；反弹入库（bank shot）加分；Fever 连击燃烧。
- **机关图鉴**：石头、冰冻（两次解冻）、星云迷雾（靠近显形）、锁链（锚定网格）、**超新星**（消除时冲击波连锁引爆同色，90 关起）、**脉冲星**（明灭周期、亮时才可消除，120 关起）、**虫洞**（弹丸穿门传送，160 关起）、**引力井**（弯曲弹道，瞄准线同步弯曲，220 关起）；弹药特殊泡：炸弹、彩虹。
- **动画手感**：被切断的泡泡是真实物理球体（重力翻滚、撞墙反弹、坠出屏幕），胜利时全场逐行倾泻；落位果冻回弹、邻泡受击颤动、墨水泼溅、屏幕震动与闪光。
- **元游戏**：19 项成就、生涯统计、星尘经济 + 12 星座图鉴（掉落泡泡产星尘，解锁点亮真实星座连线），全部本地持久化。
- **音频/触感**：全部音效与三首可切换 BGM（街机 chiptune/太空 synthwave/轻快 kawaii）均由代码合成（无任何版权素材），流式无缝循环；连击音阶逐级爬升；震动按 API 级别优雅降级。
